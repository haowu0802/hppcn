...
function P.ForeignMinister_EvaluateDecision(score, agent, decision, scope)
	...
	-- SSmith (02/07/2013)
	-- We no longer need most of this logic as we will join the Unified Front immediately
	-- Unless we have been at war with China
	...
	-- SSmith (06/05/2013)
	-- Added a new factor to encourage joining the Unified Front if ROC-Nanjing has been created
	...
	if lsDecision == "join_the_unified_front" then				-- Do we want to join the Unified Front?
		...
		-- SSmith (02/07/2013)
		-- This new code will stop us joining China quickly if we have gone to war over Guangdong!

		if country:GetRelation(chiTag):HasNap()				-- We have been at war with Chin
		and chiTag:GetCountry():GetSurrenderLevel():Get() < 0.45	-- China has a moderate Surrender Progress
...
function P.DiploScore_CallAlly(liScore, ai, actor, recipient, observer)
	...
	-- SSmith (02/07/2013)
	-- Function added to control Guangxi's response to a Chinese call to arms
	...
	if observer == recipient and tostring(actor) == "CHI" then
		...
		and actor:GetCountry():GetSurrenderLevel():Get() < 0.40		-- China has a moderate Surrender Progress
		and not njgTag:GetCountry():Exists()				-- ROC-Nanjing hasn't been created
		then
			liScore = 0
		else
